...
import styles from "./Politics.module.css"; // Changed to CSS Module
...
const Politics = () => {
  ...
  const fetchPoliticsNews = async (isInitialLoad = false) => {
    setLoading(true);
    try {
      const q = query(
      ...
      setPoliticsNews(prev => (isInitialLoad ? newsList : [...prev, ...newsList]));
  ...
  const handleNewsClick = (newsId) => {
    navigate(`/politics/${newsId}`);
  };

  return (
    <div className={styles.politicsContainer}>
      <h1 className={styles.politicsTitle}>Politics News</h1>
      ...
      {loading && <p>Loading...</p>}
      {!loading && lastVisible && (
        <button className={styles.loadMore} onClick={() => fetchPoliticsNews(false)}>Load More</button>
      )}
    </div>
  );
};
